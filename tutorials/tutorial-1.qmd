...
**Setup.** Prepare a project for the first tutorial (see the [recap if you need a refresher on how to do so](project.html)). Adopt a `tutorial-1/` root containing `data/{raw,tmp,out}`, `scripts/`, `figs/`, `tables/`, `references/`, a `README.md` and a `project.Rproj`. Working within an RStudio project sets your active working directory to the project's root folder (`tutorial-1/`).
...
::: {.qcard}
**Q9.** If you look closely at the `country` column of `gdp`, you will notice that some values do not correspond to a single country, but rather to a **group of countries** (e.g., the whole European Union, or the Euro area). Identify these aggregated observations, and remove them from `gdp` so that only individual countries remain.
...
::: {.reveal .hint-box data-reveal="1"}
Use `distinct(country)` to list the unique values taken by `country`. Among them, you should spot codes such as `EU27_2020` or `EA20`: these are Eurostat's codes for the European Union (27 members, as of 2020) and the Euro area (20 members), not for individual countries.
:::

::: {.reveal .hint-box data-reveal="2"}
Individual countries are always identified by a two-letter ISO code (e.g., `FR`, `DE`, `IT`). Aggregated groups of countries have longer codes. The base R function `nchar()` returns the number of characters in a character string — combine it with `filter()`.
:::

::: {.reveal .sol-box data-reveal="3"}
```{r}
# Inspect the distinct values of country
gdp |>
    distinct(country) |>
    print(n = Inf)

# Individual countries have a 2-character ISO code;
# groups of countries (e.g. EU27_2020, EA20) have longer codes.
gdp <- gdp |>
...
::: {.qcard}
**Q10.** In the dataset `gdp`, keep only the rows where the year is 2023. Then, sort these rows so that countries with higher values of per capita GDP appear first (i.e., arrange in descending order).
...
::: {.qcard}
**Q11.** In the dataset `gdp`, keep only the rows where the country is France.
...
::: {.qcard}
**Q12.** Adapt the following instruction to join the two datasets `gdp` and `lifesat`, using the `inner_join()` function.
...
::: {.qcard}
**Q14.** Export in a CSV file the dataset `combined_data` in your `out` data folder. Name the file `gdp-lifesat.csv`.
...
::: {.qcard}
**Q15.** If you replace `inner_join()` with `left_join()` in Q11, what would change? Explain the behavior of these two functions.
...
::: {.qcard}
**Q16.** Download an additional dataset from Eurostat. For example, the unemployment rate (ref `teilm020`). Add the values to the `combined_data` frame.
...
::: {.qcard}
**Q17. The vectorized `ifelse()` function.**
...
::: {.qcard}
**Q18. The flow-control `if` / `else` block.**
...
## Part 4 — Solving a quadratic equation with an R function
...
```{r}
quadratic_discriminant(a = 1, b = -3, c = 2)
```
...
::: {.reveal .hint-box data-reveal="1"}
Add `digits = NULL` and `verbose = FALSE` as extra parameters in the function's signature — assigning them a default value is exactly what makes them optional.
:::
...
```{r}
solve_quadratic(a = 1, b = -3, c = 1, digits = 2)
```
...
```{r}
solve_quadratic(a = 1, b = -3, c = 1, digits = 2, verbose = TRUE)
```
