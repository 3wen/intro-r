...
- Once you have downloaded the data, put it in the `data/raw/` folder of your project for session 2.
...
## The Environment: An Analogy

- Every time `R` runs, it keeps track of all the objects you have created (data, variables, functions...).
- This is called the <span class="hl-green">Global Environment</span>: think of it as a <span class="hl-purple">classroom</span>.
- Anything you write directly in your script and evaluate (outside of a function) is added to this classroom, and stays there for you to see and use.
- But what happens **inside** a function is a different story...

## Your Function is a Mixer

- Imagine your function is a <span class="hl-purple">kitchen mixer</span> sitting on the counter of the classroom.

<br>

- <span class="hl-green">Arguments</span> → the **ingredients** you pour into the mixer.
- <span class="hl-green">Body</span> → the **recipe** the mixer follows (the instructions/operations).
- <span class="hl-green">Return Value</span> → the **final product** that comes out of the mixer (a cake, or a smoothie).

::: {.warning}
<span class="box-title">Local vs Global</span>
While it is mixing, the mixer creates its own little **local environment**: a private workspace that only exists while the mixer is running.
:::

## What Stays, What Goes to the Trash

- Everything created **inside** the mixer (intermediate bowls, spoons, half-mixed batter...) only exists while the mixer is running.
- Once the mixer stops, **only the final product (the returned value) leaves the mixer** and enters the classroom (the Global Environment).
- <span class="hl-purple">Everything else created inside is thrown in the trashcan</span>: it disappears as soon as the function finishes running.

::: {.tip}
<span class="box-title">Back to our example</span>
In `add_numbers()`, the object `sum_of_numbers` is created *inside* the mixer.

- Does `sum` exist in your Global Environment after calling `add_numbers(x = 2, y = 3)`?
- What is the only thing that makes it out of the mixer?
:::
...
:::{.fragment}
To store the values through the iterations:
```{r}
#| eval: FALSE
...
for (i in 1:5) {
    j <- i^2
    res[i] <- j # the ith element gets the value j
    print(j)
}
```
:::
...
## while Loop Syntax and Example {.small}
...
```{r}
#| eval: false
# Print the squares of the integers until the current integer is lower or equal to 5
...
:::{.fragment}
To store the values of $i^2$ through the iterations:
```{r}
#| eval: false
res <- NULL # initialize object with NULL value
i <- 1 # initialize the counter to 1
while (i <= 5) {
    j <- i^2
    res <- c(res, j) # Append j to res
    print(j)
    i <- i + 1 # Increment the counter
    res
}
...
To load text files (e.g., the <a href="data/raw/age_gender.txt" class="btn btn-light btn-sm" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{< fa download >}} age_gender.txt file</a> stored in your computer, in the `data/raw` folder, use the function `read.table()`.
...
```r
tab <- read.table(file = "data/raw/age_gender.txt", header = TRUE)
head(tab, 3)
```
...
```{r}
#| eval: false
survey <- haven::read_dta("data/raw/US_labor_force_survey.dta")
```

- Put both files in the `data/raw/` folder of your project for session 2 before importing them.
...
universities <- read_csv("data/raw/HD2018.csv")
...
universities <- universities |> 
  mutate(
    ...
    # Remove the first occurence of " COUNTY" in column COUNTYNM
    COUNTYNM = str_remove(COUNTYNM, " COUNTY")
  )
